...
<template>
	<div data-e2e-id="graphql-connected">
		...
		<p>
			...
			devtools network tab. Note that Apollo Client maintains a query cache - so
			the same query will not execute twice (i.e. after route changes) unless
			either the page is refreshed, or the <em>fetch policy</em> is set to not
			use the cache. Consult the Apollo documentation for details.
		</p>
		...
		<div v-if="!loading && result && result.datasource">
			...
			<br />
			sample1 (editable):
			<sc-text :field="result.datasource.sample1.jsonValue" />
			<br />
			sample2:<br />
			<ul>
				...
				<li>
					editable: <sc-link :field="result.datasource.sample2.jsonValue" />
				</li>
				<li>field type: {{ result.datasource.sample2.definition.type }}</li>
				<li>
					field is shared?:
					{{ result.datasource.sample2.definition.shared.toString() }}
				</li>
			</ul>
		</div>
		<div v-if="!loading && result && result.contextItem">
			...
			<ul>
				<li
					v-for="child in result.contextItem.children.results"
					:key="child.id"
				>
					<router-link :to="child.url.path">{{
						child.pageTitle.value
					}}</router-link
					>&nbsp; (editable title too!
					<sc-text :field="child.pageTitle.jsonValue" />)
				</li>
			</ul>
		</div>
	</div>
		...
		const variables = () => {
			...
			const variables = {
				contextItem: properties
					? properties.sitecoreContext().itemId
					: defaultValue,
				datasource:
					(props.rendering && props.rendering.dataSource) || defaultValue,
				language: properties
					? properties.sitecoreContext().language
