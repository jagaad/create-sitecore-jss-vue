...
<template>
	<styleguide-specimen
		v-bind="$props"
		data-e2e-id="styleguide-fieldusage-content-list"
	>
    ...
    -->
		<template v-if="fields.sharedContentList">
			<div
				v-for="(listItem, index) in fields.sharedContentList"
				:key="`sharedListItem-${index}`"
			>
		...
		<template v-if="fields.localContentList">
			<div
				v-for="(listItem, index) in fields.localContentList"
				:key="`localListItem-${index}`"
			>
				<!-- The referenced item's fields can be rendered and edited using normal helper components: -->
				<p>Field: <sc-text :field="listItem.fields.textField" /></p>
			</div>
